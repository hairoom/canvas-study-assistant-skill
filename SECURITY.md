...
资源搜索无可靠结果时可以自动进入只读发现模式，但必须限制为当前 Canvas 域名下以 `/api/` 开头的 GET 请求。禁止在路径或参数中携带 Token，限制分页和返回数量，并且只有返回了与用户查询相关的课程资源时才能保存本地发现规则。规则按 Canvas 域名和课程隔离。官方文档检索不得携带 Canvas 凭证，动态发现不得学习或执行新的写操作。
