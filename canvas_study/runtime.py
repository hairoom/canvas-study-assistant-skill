#!/usr/bin/env python3
"""Shared Canvas runtime with safe credentials, discovery, transfers, and CLI fallback."""
...
def application():
    """Build the shared application service against this local runtime."""
    from canvas_study.application import CanvasApplication
    return CanvasApplication(sys.modules[__name__])


def cmd_status(args):
    output(application().connection_status())


def cmd_index_sync(args):
    output(application().sync_courses())


def cmd_index_status(args):
    output(application().sync_status())


def cmd_find_resource(args):
    output(application().find_resource(args.course, args.query, args.kind or None, args.limit))


def cmd_course_tree(args):
    output(application().course_tree(args.course))


def cmd_courses(args):
    output(application().list_courses(args.refresh)["courses"])


def cmd_assignments(args):
    output(application().list_assignments(args.course, args.pending, args.refresh))


def cmd_schedule(args):
    output(application().schedule(args.days))
...
def cmd_files(args):
    output(application().get_files(args.course, args.refresh))


def cmd_modules(args):
    output(application().get_modules(args.course, args.refresh))


def cmd_inspect(args):
    output(application().inspect_course(args.course, args.full, args.refresh))
...
def cmd_download(args):
    output(application().download_file(args.file_id, args.output))
...
def cmd_upload(args):
    if args.confirm is None:
        return output(application().preview_upload(args.course, args.assignment, args.file))
    output(application().upload_draft(args.course, args.assignment, args.file, args.confirm))
...
def cmd_preview(args):
    output(application().preview_submission(args.course, args.assignment, args.file_id))


def cmd_submit(args):
    output(application().submit_assignment(
        args.course, args.assignment, args.file_id, args.confirm,
    ))
