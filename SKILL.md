...
Use the MCP tools as the normal Agent interface for deterministic Canvas work. The CLI is a local bootstrap, recovery, and developer fallback over the same application service; do not make the user run commands during an ordinary MCP-backed workflow. Prefer indexed search before broad API scans, keep payloads small, and present results in the user's language. This skill supports student accounts only.

## First use

Keep normal initialization to two user-response rounds and at most three if recovery is needed.

If the Canvas MCP tools are unavailable, run `python scripts/setup_mcp.py` from this skill directory. It creates a persistent isolated environment, installs the MCP dependency, verifies the server, and registers it with Codex. Tell the user to restart Codex and continue in a new conversation; registration cannot add tools to a conversation that is already running. Use `--codex-bin PATH` only when the Codex executable cannot be discovered. Do not request a Canvas token during MCP installation.
...
After connection, build a metadata-only structure index for active student courses with `canvas_sync_courses` (or the CLI fallback) unless the user requests recovery mode. Say that standard caching and structural indexing are enabled: the index stores resource identifiers, titles, types and relationships, but not tokens, signed download URLs, file contents, or full content bodies. Deadlines are refreshed before planning, download URLs before download, and assignment/submission state before and after submission. Users may say “刷新 Canvas 数据”, “每次获取最新数据”, “减少 API 请求”, “恢复标准模式”, or “清空 Canvas 缓存”. Use the corresponding MCP tool or CLI recovery command.
...
- Connection, token replacement, caching, MCP/CLI operation, or API errors: read [references/api-workflows.md](references/api-workflows.md).
...
- Before any Canvas upload, call `canvas_preview_upload`, describe what will be uploaded, and obtain confirmation before `canvas_upload_draft`.
- Before formal submission, call `canvas_preview_submission`, show course, assignment, file, deadline, current time, late/locked state, and attempt impact, then obtain explicit final confirmation before `canvas_submit_assignment`.
